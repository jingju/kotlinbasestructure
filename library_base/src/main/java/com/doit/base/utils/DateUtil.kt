...
public object DefaultDateFormat {
    ...
    val DEFAULT_FORMAT =
        ThreadLocal<SimpleDateFormat>().apply { set(SimpleDateFormat(DEFAULT_DATE_STR, Locale.US)) }
}
...
fun formatAgoStyleForWeibo(
    time: String,
    format: DateFormat = DefaultDateFormat.DEFAULT_FORMAT.get()!!
): String = parseDateString2Mills(time, format).formatAgoStyleForWeibo()
...
fun formatAgoStyleForWeChat(
    time: String,
    format: DateFormat = DefaultDateFormat.DEFAULT_FORMAT.get()!!
): String = parseDateString2Mills(time, format).formatAgoStyleForWeChat()
